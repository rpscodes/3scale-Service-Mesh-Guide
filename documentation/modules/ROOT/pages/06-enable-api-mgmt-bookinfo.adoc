...
. Create a Custom Resource Definition file for the Backend Service Entry with name 'ServiceMeshExtension_bookinfo.yaml' using vim or any other editor on the CLI. Copy paste the below yaml into the file and replace the access token, service token, product id values and shown in the image below and save it. 
+
[.lines_7]
[.console-input]
[source, yaml,subs="+macros,+attributes"]
...
. You should see 403 status and we have not provided an api user key.
+
image::403.png[]

. Now let's append the api key and see what happens. Copy and paste the below command in reqbin. Do not forget to Replace the *cluster wildcard url*  and *userkey*. 
+
[.lines_7]
[.console-input]
[source, bash,subs="+macros,+attributes"]
