...
. Create a Request Authentication Custom Resource Definition file with name 'RequestAuthentication_bookinfo-oidc.yaml' using vim or any other editor on the CLI. Copy paste the below yaml into the file and save it. Do not forgot to replace the keycloak placeholders with the URL obtained in the previous step
+
[.lines_7]
[.console-input]
[source, yaml,subs="+macros,+attributes"]
...
. The service token wil enable the permission for service mesh to be able to access a particular 3scale product. Use 3scale admin access token along with the 3scale product ID from the 3scale product configuration earlier and run the following command with values replaced to obtain the Service token. Do not forget to Replace the *cluster wildcard url*:
+
[.lines_7]
[.console-input]
[source, bash,subs="+macros,+attributes"]
...
. Access with JWT token using Client Credential flow using the Client ID, Client Secret and Keycloak URL obtained in the previous steps. Replace the 'Client ID' 'Client Secret', 'Keycloak URL' and 'cluster wildcard' in the commands below:
+
[.lines_7]
[.console-input]
[source, bash,subs="+macros,+attributes"]
...
----
http://istio-ingressgateway-istio-system.{cluster wildcard url}/api/v1/products
----

. Click on Authorization and select OAUth 2.0 from the Type drowndown
+
...
. Make sure you choose and fill in the correct values as shown below.
 ...
 - Auth URL: https://<Replace with Keycloak URL>/auth/realms/threescale/protocol/openid-connect/auth
 - Access Token URL: https://<Replace with Keycloak URL>/auth/realms/threescale/protocol/openid-connect/auth
...
. The browser shoudl open a login window asking for username and password. Go ahead and enter user1 for the username and openshift for the password
