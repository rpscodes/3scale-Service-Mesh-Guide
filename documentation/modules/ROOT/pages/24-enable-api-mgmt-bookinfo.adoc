...
[NOTE]
===============================
Alternatively you can use 3scale admin access token along with the 3scale product ID from the 3scale product configuration earlier and run the following command with values replaced to obtain the Service token. Do not forget to Replace the *cluster wildcard url*:
+
[.lines_7]
[.console-input]
[source, bash,subs="+macros,+attributes"]
